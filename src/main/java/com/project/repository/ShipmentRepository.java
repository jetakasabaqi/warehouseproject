...
public interface ShipmentRepository extends JpaRepository<Shipment, Long>, JpaSpecificationExecutor {
    ...
    @Query("select new com.project.service.dto.PackageDTO(p.id,s.id,r.id,pr.id,s.statusName,r.fullName,r.address,r.zipCode,pd.width,pd.length,pd.height,pd.weight,wh.unit,pt.type,pr.price)" +
        ...
        " inner join pd.product p " +
        " inner join p.price pr " +
        " where senderp_id=:senderp_id and p.id=:product_id")
    PackageDTO getAllByPersonAndProduct(@Param("product_id") Long productId, @Param("senderp_id") Long personId);
    ...
    @Query("select new com.project.service.dto.PackageStatusDTO(p.id,pr.id,s.id,e.id,e.name,s.statusName,pr.price)" +
        ...
        " inner join p.price pr" +
        " where senderp_id=:senderp_id and p.id=:product_id")
    PackageStatusDTO getPackageDetails(@Param("senderp_id") Long senderpId, @Param("product_id") Long productId);
    ...
    PackageInfoDTO getPackageInfo(@Param("person_id") Long personId, @Param("product_id") Long productId);
    ...
    @Query("select new com.project.service.dto.NoOfPacksDeliveredDTO(count(sh))" + "" +
        " from Shipment sh " + "" +
        " inner join sh.status s where s.id=4")
    NoOfPacksDeliveredDTO getNoOfPacksDelivered();

    @Query("select new com.project.service.dto.NoOfPacksDeliveredDTO(count(sh)) from Shipment sh" +
        " inner join sh.status s " +
        " inner join sh.senderP p " +
        " where s.id=4 and p.country=:country")
    NoOfPacksDeliveredDTO getNoOfPacksDeliveredByCountry(@Param(value = "country") String country);

    @Query("select new com.project.service.dto.NoOfPacksPendingDTO(count(sh),r.country) from Shipment sh " +
        " inner join sh.status s " + "" +
        " inner join sh.receiver r " +
        " where s.id=2 group by r.country")
    List<NoOfPacksPendingDTO> getNoOfPacksPending(Pageable pageable);

    @Query("select new com.project.service.dto.LoyalClientsDTO(sh.senderP.fullName,sh.senderP.tel,sh.senderP.address,count(sh.senderP)) from Shipment sh " +
        " group by (sh.senderP.fullName,sh.senderP.tel,sh.senderP.address) " +
        " having count(sh.senderP)>=4")
    List<LoyalClientsDTO> getLoyalClients(Pageable pageable);

    @Query("select new com.project.service.dto.NoOfPackByAnyCountryDTO(count(sh),p.country) from Shipment sh " +
        " inner join sh.status s " +
        " inner join sh.senderP p " +
        " where s.id=4 group by p.country")
